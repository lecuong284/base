...
All Notable changes to `Lecuong\Base` will be documented in this file.
...
### Fixed
- language files for all Lecuong packages are now loaded by Lecuong\Base, using pointer files; this fixes the language fallback system;
- Lecuong\Base language files no longer need publishing;
...
### Added
- lecuong/generators require-dev dependency;
...
### Removed
- Lecuong/Base DOES NOT automatically include lecuong/generators and laracasts/generators on --dev; composer does not permit installing require-dev dependencies of dependencies;


## [0.6.15] - 2016-08-30

### Added
- Lecuong/Base automatically includes lecuong/generators and laracasts/generators on --dev.
...
- Lecuong/Base no longer uses any routes outside the /admin/ prefix in order not to conflict with any catch-all routes (as developers will probably use with PageManager).
...
- All routes now use the /admin/ prefix, even the admin authentication. This separates LeCuong completely from the main application and allows it to have a different auth for users, if needed.
...
### Added
- AdminLTE usage in all auth views. LeCuong is pretty now.
...
### Added
- Moved all controllers, views, config file, lang file for Laravel authentication into the package. Loading the package will allow the user to make use of Lecuong authentication, instead of Laravel's default.
